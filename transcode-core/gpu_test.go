...
func TestFilterNVIDIACapabilities_DisablesAV1OnRTX2080(t *testing.T) {
	hw := HWProfile{
		GPUName:  "NVIDIA GeForce RTX 2080",
		Vendor:   VendorNVIDIA,
		Encoders: []string{"h264_nvenc", "hevc_nvenc", "av1_nvenc"},
		Decoders: []string{"h264_cuvid", "hevc_cuvid", "av1_cuvid"},
	}
	hw = filterNVIDIACapabilities(hw)
	if hw.HasEncoder("av1_nvenc") {
		t.Fatal("expected av1_nvenc to be removed for RTX 2080")
	}
	if hw.HasDecoder("av1_cuvid") {
		t.Fatal("expected av1_cuvid to be removed for RTX 2080")
	}
}

func TestFilterNVIDIACapabilities_KeepsAV1OnRTX4090(t *testing.T) {
	hw := HWProfile{
	...
	hw = filterNVIDIACapabilities(hw)
	if !hw.HasEncoder("av1_nvenc") {
		t.Fatal("expected av1_nvenc to be kept for RTX 4090")
	}
	if !hw.HasDecoder("av1_cuvid") {
		t.Fatal("expected av1_cuvid to be kept for RTX 4090")
	}
}

func TestDetectNVIDIACapabilityProfile(t *testing.T) {
	tests := []struct {
		name     string
		gpuName  string
		gen      NVIDIAGeneration
		hasAV1   bool
	}{
		{"GTX 1080", "NVIDIA GeForce GTX 1080", NVIDIAGenerationPascal, false},
		{"RTX 2080", "NVIDIA GeForce RTX 2080", NVIDIAGenerationTuring, false},
		{"RTX 3090", "NVIDIA GeForce RTX 3090", NVIDIAGenerationAmpere, false},
		{"RTX 4090", "NVIDIA GeForce RTX 4090", NVIDIAGenerationAda, true},
		{"L4", "NVIDIA L4", NVIDIAGenerationAda, true},
		{"Unknown", "Some Unknown GPU", NVIDIAGenerationUnknown, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			profile := DetectNVIDIACapabilityProfile(tt.gpuName)
			if profile.Generation != tt.gen {
				t.Fatalf("generation = %q, want %q", profile.Generation, tt.gen)
			}
			if profile.SupportsAV1 != tt.hasAV1 {
				t.Fatalf("SupportsAV1 = %v, want %v", profile.SupportsAV1, tt.hasAV1)
			}
		})
	}
}
